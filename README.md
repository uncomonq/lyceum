...
- Пользователь может поставить товару одну оценку от 1 до 5;
  - Значения: `1 — Ненависть`, `2 — Неприязнь`, `3 — Нейтрально`, `4 — Обожание`, `5 — Любовь`;
  - Оценка необязательна (можно удалить свою оценку).
- В карточке товара (`/catalog/<id>/`) отображаются:
  - Средняя оценка;
  - Количество оценок;
  - Для авторизованного пользователя — его текущая оценка и форма изменения/удаления.

## Статистика

- `/statistics/users/` — статистика по пользователям (лучший/худший товар, количество и средняя оценка);
- `/statistics/items/` — статистика по товарам (средняя оценка, количество, последняя максимальная/минимальная оценка);
- `/statistics/my-items/` — список товаров, оценённых текущим пользователем, с сортировкой по оценке по убыванию
...
- `rating_rating` — оценки товаров (хранит значение оценки);
