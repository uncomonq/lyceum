...
> Файл `.env` добавлен в `.gitignore`, поэтому не попадает в репозиторий.
> В `settings.py` заданы рабочие значения по умолчанию для локального запуска,
> поэтому проект стартует даже без `.env`.
